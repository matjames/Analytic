...
func TestStatChatIntegrationForwardsAIIdentity(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if input.ObjectRef != "obj:ai-autonomy:agent:agent-1" {
	...
	conversation, err := NewStatChatIntegration(server.URL, "internal-key").ensureDiscussion(context.Background(), "Bearer token", "user-1", "tenant-1", "workspace-1", "agent", "agent-1", "Agent")
	if err != nil {
		t.Fatal(err)
	}
