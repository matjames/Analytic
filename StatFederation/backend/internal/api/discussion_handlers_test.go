...
func TestStatChatIntegrationForwardsFederationIdentity(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if input.ObjectRef != "obj:statfederation:federated_node:node-1" {
	...
	defer server.Close()
	conversation, err := NewStatChatIntegration(server.URL, "internal-key").ensureDiscussion(context.Background(), "Bearer token", "user-1", "tenant-1", "workspace-1", "federated_node", "node-1", "Node")
	if err != nil {
		t.Fatal(err)
	}
