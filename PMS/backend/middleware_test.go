...
func pmsTestToken(secret, role, tenant string) string {
...
func TestRegistryAuthMiddlewareConverged(t *testing.T) {
	secret := "test-pms-secret-32-chars-long-!!"
	...
	r.ServeHTTP(rr, httptest.NewRequest("GET", "/api/v1/projects", nil))
	if rr.Code != http.StatusUnauthorized {
		t.Errorf("expected 401 unauthenticated, got %d", rr.Code)
	}

	// 2. Valid Registry token -> 200
	tok := pmsTestToken(secret, "admin", "uganda-national")
