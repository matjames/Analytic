...
func TestDataObjectReference(t *testing.T) {
	if got := dataObjectReference("dataset", "dataset-1"); got != "obj:statdata:dataset:dataset-1" {
		t.Fatalf("unexpected object reference: %q", got)
	}
}

func TestStatChatIntegrationForwardsDataIdentity(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Header.Get("X-Internal-API-Key") != "internal" || r.Header.Get("X-StatGate-User-ID") != "user-1" || r.Header.Get("X-Tenant-ID") != "tenant-1" || r.Header.Get("X-Workspace-ID") != "workspace-1" {
			t.Fatalf("data identity context was not forwarded")
		}
		var input statchat.ObjectConversationRequest
		if err := json.NewDecoder(r.Body).Decode(&input); err != nil {
			t.Fatalf("decode request: %v", err)
		}
		if input.ObjectRef != "obj:statdata:dataset:dataset-1" {
	...
	conversation, err := integration.ensureDiscussion(context.Background(), "user-1", "tenant-1", "workspace-1", "dataset", "dataset-1", "Dataset 1")
