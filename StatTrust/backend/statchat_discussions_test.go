...
func TestStatTrustStatChatForwardsIdentity(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if input.ObjectRef != "obj:stattrust:incident:incident-1" {
	...
	defer server.Close()
	conversation, err := newStatTrustStatChat(server.URL, "internal-key").ensureDiscussion(context.Background(), "Bearer token", "user-1", "tenant-1", "workspace-1", "incident", "incident-1", "Incident")
	if err != nil {
		t.Fatal(err)
	}
