...
import {
  PublicFacilityLevelsApi,
  PublicFacilitiesApi,
  PublicOwnershipTypesApi,
  PublicAuthorityTypesApi,
  PublicAdminUnitsApi,
  PublicAdminLevelsApi,
} from "../../helpers/api";
...
const PublicMasterFacilityList = () => {
  ...
  useEffect(() => {
    ...
    if (filterRegion) {
      ...
      PublicAdminUnitsApi.list().then(adminUnits => {
  ...
  async function loadLookups() {
    try {
      const [levels, ownerships, authorities, adminUnits, levelsData, treeRes] = await Promise.all([
        PublicFacilityLevelsApi.list(),
        PublicOwnershipTypesApi.list(),
        PublicAuthorityTypesApi.list(),
        PublicAdminUnitsApi.list(),
        PublicAdminLevelsApi.list().catch(() => []), // Fallback to empty array if fails
        PublicAdminUnitsApi.tree().catch(() => ({ tree: [] })), // Fallback if fails
      ]);
  ...
  async function loadFacilities(nextPage = 1) {
    ...
    setError("");
    try {
      const res = await PublicFacilitiesApi.listPaged({
  ...
  const handleViewFacility = (facility) => {
    if (!facility?.mfl_uid && !facility?.identifier && !facility?.id) return;
    history.push(`/mfl/${facility.mfl_uid || facility.identifier || facility.id}`);
  };
  ...
  const handleExport = async () => {
    try {
      ...
      const rows = await PublicFacilitiesApi.export(params);
  ...
  // Status helpers are kept for possible future re-introduction of the status column
      ...
      <div className="container">
        <div className="page-header">
          <div>
            ...
            <p>Browse registered field survey stations, agent coverage, and operational territories across the country.</p>
          </div>
        </div>
        ...
        <div className="filter-section">
          ...
          <div className="filter-row mfl-filters-row">
              <div className="filter-group">
                <label>Search Field Station</label>
                <input
                  type="text"
                  placeholder="Enter station name or registry ID..."
                  value={searchQuery}
              ...
              <div className="filter-group">
                ...
                <select value={filterFacilityLevelId} onChange={handleLevelChange}>
                  <option value="">All Station Tiers</option>
...
export default PublicMasterFacilityList;
