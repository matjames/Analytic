...
export default function FacilityList() {
  ...
  const [filterFacilityLevelId, setFilterFacilityLevelId] = useState("");
  ...
  useEffect(() => {
    ...
    const subcountyParam = params.get("subcounty");
    const reportingParam = params.get("reporting");
    
    if (subcountyParam) {
      setFilterSubcounty(subcountyParam);
    } else {
      setFilterSubcounty("");
    }
    
    if (reportingParam) {
      // Convert "yes"/"no" to "true"/"false" if needed, or use as-is
      if (reportingParam === "yes") {
        setFilterReporting("true");
      } else if (reportingParam === "no") {
        setFilterReporting("false");
      } else {
        setFilterReporting(reportingParam);
      }
    } else {
      setFilterReporting("");
    }
  ...
  async function loadLookups() {
    try {
      ...
      // Debug logging
      console.log("Loaded data:", { levels, ownerships, authorities, adminUnits, levelsData, treeRes });
  ...
  async function loadFacilities(nextPage = 1) {
    ...
    try {
      ...
      setError(e?.response?.data?.error || e.message || "Failed to load field stations");
    } finally {
      setLoading(false);
    }
  }
  ...
  const formatIdentifier = (identifier) => {
    if (!identifier || typeof identifier !== "string") return formatValue(identifier);
  ...
  const handleLevelChange = (e) => {
    setFilterFacilityLevelId(e.target.value || "");
  };
  ...
  const handleReportingChange = (e) => {
    setFilterReporting(e.target.value || "");
  };

  const handleClearFilters = () => {
    setSearchQuery("");
    setFilterFacilityLevelId("");
    setFilterOwnership("");
    setFilterAuthority("");
    setFilterReporting("");
    setFilterRegion("");
  ...
  const handleViewFacility = (facility) => {
    if (!facility?.mfl_uid) {
      console.error("Cannot view facility: mfl_uid is missing", facility);
      return;
    }
    history.push(`/facilities/${facility.mfl_uid}`);
  };

  const stats = facilities.reduce(
    (acc, f) => {
      const status = (f.status || "").toLowerCase();
      if (status === "functional" || status === "active") acc.active += 1;
      else if (status === "pending") acc.pending += 1;
      else if (status === "non-functional" || status === "inactive") acc.inactive += 1;
      return acc;
    },
  ...
  const buildQueryParams = (pageOverride) => {
    ...
    const params = {
      page: nextPage,
      pageSize,
      ...(filterFacilityLevelId && { level: filterFacilityLevelId }),
    ...
    if (filterReporting === "true" || filterReporting === "false") {
      params.reporting = filterReporting;
    }
  ...
  const handleExport = async () => {
    try {
      setExporting(true);
      // Request a large pageSize to effectively get all facilities for this user/role + filters
      const params = {
        ...buildQueryParams(1),
        pageSize: 100000,
      };
      ...
      const header = [
        // All columns from mfl_details, excluding id and user_id and mfl_uid
        "identifier",
        "name",
        "short_name",
        "historical_id",
  ...
  const getStatusBadgeClass = (status) => {
    ...
    if (statusLower === "functional" || statusLower === "active") {
      return "status-badge status-active";
    }
    if (statusLower === "pending") {
      return "status-badge status-pending";
    }
    if (statusLower === "non-functional" || statusLower === "inactive") {
  ...
  return (
    <Fragment>
      <div className="page-header">
        <div>
          <h2>Field Operations Registry</h2>
          <p>Browse and manage field survey stations across the country.</p>
        </div>
      </div>
      ...
      <div className="filter-section">
        ...
        <div className="row">
            <div className="col-3 filter-group">
              ...
              <input
                type="text"
                placeholder="Enter field station name or code..."
                value={searchQuery}
            ...
            <div className="col-3 filter-group">
              <label>Level</label>
              <select value={filterFacilityLevelId} onChange={handleLevelChange}>
          ...
          <tbody>
            ...
            {facilities.map((facility, index) => (
              <tr key={facility.id || facility.identifier || index}>
                ...
                <td>{formatValue(facility.name)}</td>
                <td>{formatValue(facility.level)}</td>
                <td>{formatValue(facility.ownership)}</td>
                <td>{formatValue(facility.authority)}</td>
