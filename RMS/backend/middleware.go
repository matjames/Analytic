...
func registryAuthMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Fail closed (SG-SEC-2026-08): no hardcoded default secret.
		...
		issuer := rmsEnv("STATGATE_JWT_ISSUER", "statgate-registry")
		audience := rmsEnv("STATGATE_JWT_AUDIENCE", "statgate")
		...
		c.Set("email", rmsClaimString(claims, "email"))

		c.Next()
	}
}

// rmsValidRole is the canonical role whitelist shared across services.
func rmsValidRole(role string) bool {
...
// rmsClaimString reads a string claim under any of the given names.
func rmsClaimString(claims jwt.MapClaims, names ...string) string {
...
// rmsEnv reads an env var with a fallback.
func rmsEnv(key, def string) string {
