...
# JupyterHub now authenticates every user through the StatGate Registry
# identity layer. If STATGATE_REGISTRY_API_URL is missing, the Hub refuses
...
# ── Session security ──────────────────────────────────────────────
# Cookie secret is injected at runtime. Never use a default value.
cookie_secret = os.getenv("JUPYTERHUB_COOKIE_SECRET", "").strip()
if not cookie_secret:
    # Fail closed: refuse to run with a fixed/predictable secret.
    raise SystemExit(
        "FATAL: JUPYTERHUB_COOKIE_SECRET must be set. Refusing to start."
    )
