...
- Extend the now-certified PMS/RMS, Enterprise workflow, and StatCollect survey object-conversation pattern to field, governance, and later modules.
- Standardize event names, payloads, retries, idempotency, and dead-letter handling; StatCollect now emits the shared versioned envelope, while durable delivery and downstream consumers remain.
