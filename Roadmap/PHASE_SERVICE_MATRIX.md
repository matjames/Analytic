...
**Resource-level scoping progress:** ✅ **statdata datasets + pipelines DONE 2026-08-29** — `WorkspaceID` on `models.Dataset`/`models.DataPipeline`; `ListDatasets`/`ListPipelines` filtered by selected workspace (pg + mem stores); create stamps workspace from context; read/trigger/delete enforce ownership (404 on cross-workspace, legacy unscoped rows stay visible and are adopted on first edit); `statdata.datasets`/`statdata.pipelines` self-provisioned with `workspace_id` via `PGStore.ensureSchema` (discovered + fixed latent gap: statdata schema was never provisioned — tables were absent entirely on live and clean volumes). Unit tests: `workspace_scope_test.go`. Replication targets: remaining statdata domains (data sources, contracts, quality, streaming, feature store) then other services.
