...
2. **Stage 2 — Identity/security closure:** ✅ **ROLLED OUT 2026-08-29 to all 10 gap services** — shared middleware in `statgate-lib/tenant` (gin variants `GinWorkspaceContext`/`GinWorkspaceMembership`; net/http variants `WorkspaceContext`/`WorkspaceMembership` for gorilla/mux services; `VerifyWorkspaceMembership` helper; 12 unit tests). Adopted by: **statdata, statiot, geointel, ai-autonomy, learning-crm, bpm-hub, knowledge-portal, statgate-enterprise (search), statgate-integration-fabric, statgate-runops-api** — all rebuilt, redeployed, healthy (45/45 containers), live-verified (malformed X-Workspace-ID → 400; pass-through without credentials; membership 403/503 mapping unit-tested). Dockerfile/compose notes: enterprise search + integration Dockerfiles rewritten to repo-root context; STATGATE_ENTERPRISE_API_URL wired into all 10. **Remaining Stage 2 follow-ups:** resource-level workspace predicates (PMS-style `workspace_id=$2` store scoping) per service; statiot's AuthValidator configured but not applied to routes (device-token routes need an auth decision); tenant-branding adoption by later-phase UIs.

**Resource-level scoping progress:** ✅ **statdata datasets + pipelines DONE; statiot gateways/devices/alerts/field-workers DONE (2026-08-29)** — `WorkspaceID` on models; list filters + create stamps + read/trigger/delete ownership checks (404 on cross-workspace; legacy unscoped rows stay visible and are adopted on first edit); statdata tables self-provisioned via `PGStore.ensureSchema`; statiot gained a migration runner (`ensureMigrations` executes bundled `migrations/001_init.sql` + adds `workspace_id` columns — fixed latent gap: statiot's DB had 0 tables; now 18 tables + 4 workspace columns provisioned on live volume). Unit tests: `workspace_scope_test.go` (statdata). Replication targets: remaining statdata domains (data sources, contracts, quality, streaming, feature store), then geointel/bpm-hub/learning-crm/ai-autonomy/knowledge-portal stores.
3. **Stage 3 — Cross-module collaboration:** object conversations for field/survey/governance modules; standardize event names, retries, idempotency, DLQ.
