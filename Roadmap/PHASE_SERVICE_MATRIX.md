...
2. **Stage 2 — Identity/security closure:** ✅ **STARTED 2026-08-29** — shared workspace middleware added to `statgate-lib/tenant` (`GinWorkspaceContext`, `GinWorkspaceMembership`, `VerifyWorkspaceMembership`, unit-tested). **Pilots adopted and deployed: statdata, statiot** (CORS + context + Enterprise Core membership; `STATGATE_ENTERPRISE_API_URL` wired in compose; live-verified: malformed X-Workspace-ID → 400, pass-through without credentials → 200, membership 403/503 mapping unit-tested). **Remaining services needing adoption:** geointel, ai-autonomy, learning-crm, bpm-hub, knowledge-portal, enterprise/search, enterprise/integration, PlatformEngineering (runops). Also noted: statiot's `AuthValidator` is configured but not applied to routes (device-token routes need a deliberate auth decision); resource-level workspace predicates (the PMS-style `workspace_id=$2` scoping) are the follow-up per service.
