...
**Resource-level scoping progress:** ✅ **statdata datasets + pipelines; statiot gateways/devices/alerts/field-workers; geointel geo_layers; bpm-hub process_definitions/process_instances DONE (2026-08-29)** — list filters + create stamps + ownership guards (404 on cross-workspace; legacy unscoped rows stay visible and are adopted on first edit); schema columns self-provisioned (statdata `ensureSchema`, statiot `ensureMigrations` runner — fixed latent gaps where statdata/statiot DBs had no tables; geointel/bpm-hub ALTERs via ensureSchema). Unit tests: `workspace_scope_test.go` (statdata). Replication targets: remaining statdata domains (data sources, contracts, quality, streaming, feature store), learning-crm, ai-autonomy, knowledge-portal stores. Note: geointel REDIS_ADDR not set in compose (event bus falls back to in-memory) — follow-up.
