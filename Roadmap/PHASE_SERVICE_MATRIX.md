...
**Live certification update — 2026-09-02:** StatData full implemented schema parity and authenticated science/search/compliance write/read paths are certified. Workspace boundaries now cover all implemented StatData domains, including deep reads, search, saved searches, audit logs, object links, and science execution paths; legacy rows without a workspace remain tenant-wide for compatibility and cross-workspace rows are denied. Two-workspace live isolation certification passed. All ten audited later services pass authenticated seeded-object StatChat discussion write/read certification in the real `phase3-cert` workspace. Enterprise Core JWT/database workspace configuration is repaired; IoT workspace ownership persistence and CRM/BPM workspace projection scans were corrected. Durable interruption/reclaim and Redis stop/start failover now pass isolated live certification. StatOps now publishes/consumes canonical durable operations events through the named `statops` group, preserves legacy-envelope compatibility, and filters centralized logs by tenant plus workspace. Enterprise Core now consumes canonical durable events through the named `enterprise-core` group, adapts them into its existing projections, and preserves legacy compatibility; acknowledgement/idempotency certification passed for both consumers. Remaining gates are later-module frontend entry points, broader durable consumer rollout, and browser/NAT/TURN production certification.
...
3. **Stage 3 — Cross-module collaboration:** **PARTIAL** — PMS/RMS, Enterprise workflows, StatCollect surveys, StatGovernance core objects, StatIoT/field API objects, StatData API objects, GeoIntel objects, AI Autonomy objects, Learning/CRM objects, BPM objects, StatFederation objects, StatSpatial objects, and StatTrust objects now use canonical StatChat object conversations at their authenticated API boundaries. All ten later-service discussion routes are live-probed and fail-closed without credentials; authenticated seeded-object writes/read-back are certified, while later-module frontend sources remain. StatCollect and StatIoT use the shared durable publisher; StatIoT, StatData, AI Autonomy, StatFederation, StatTrust, and StatOps consume durable Streams with named groups. StatOps centralized logs are tenant/workspace filtered and retain compatibility with older event envelopes. Live Redis stream consumption, retry-to-DLQ, abandoned-consumer reclaim, and stop/start failover are certified; broad durable consumer rollout remains. **Event-bus standardization audit (2026-08-29):** docs/EVENT_CATALOG.md extended with sections 2.10–2.16 for all later-phase domains; fixed a taxonomy violation where statiot's `PublishTelemetryIngested` emitted `dataset.updated` (statdata indexed every telemetry batch as a dataset) — now emits `telemetry.ingested` with the device's real tenant.
...
StatCitizen is now standardized on the shared durable event publisher and is live-ready on port `8115`. Production secret wiring, Postgres role/database provisioning, startup migrations, Redis durable-bus connection, Enterprise Core registration, and `/health` plus `/ready` checks are certified.

### Live Recovery Checkpoint: 15 September 2026

After Docker Desktop recovery and Postgres WAL startup, the complete Analytic container scan is clean: no service is starting, unhealthy, restarting, or exited. StatCitizen is running with restart count `0`; its health/readiness checks and Helpdesk API documentation endpoint return `200`. Browser E2E and the normal StatCitizen image build remain external/dependency-network gates.
...
PMS portfolio/programme aggregation is also certified through `GET /api/portfolio-dashboard`: workspace-scoped KPIs, stage distribution, group rollups, project rows, and portfolio/programme filters are live in the global UI. The PMS-mediated `POST /api/projects/:id/health-assistant` endpoint is certified for governed risk, schedule, and milestone advisory with invalid-action and foreign-workspace denial checks. StatSpatial project-location save/readback is now certified with workspace isolation and coordinate validation; PMS budget forecasting remains open.
