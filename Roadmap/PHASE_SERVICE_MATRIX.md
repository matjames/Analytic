...
**Resource-level scoping progress:** ✅ **statdata datasets domain DONE 2026-08-29** — `models.Dataset.WorkspaceID`; `ListDatasets` filtered by selected workspace (pg + mem stores); create stamps workspace from context; read/update/delete enforce ownership (404 on cross-workspace, legacy unscoped rows stay visible and are adopted on first edit); `statdata.datasets`/`statdata.pipelines` self-provisioned with `workspace_id` via `PGStore.ensureSchema` (discovered + fixed latent gap: statdata schema was never provisioned — tables were absent entirely on live and clean volumes). Unit tests: `workspace_scope_test.go`. Replication targets: remaining statdata domains (pipelines CRUD, data sources, contracts, quality, streaming, feature store) then other services.
