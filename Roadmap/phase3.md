...
- **Competitive collaboration tools** — collaborative documents, whiteboards, translation, explicit social-graph requests, wellness interactions, and URL-backed photo/video/article feed posts are implemented; dedicated media-provider/upload UX remains a production enhancement.
...
- [x] Frontend wired to supported product APIs — repeatable audit verifies 150 typed frontend operations against 162 routes; the 12 uncovered routes are health/readiness, legacy aliases, upload serving, and duplicate task aliases. Secure group/channel member administration is exposed only when the server reports owner/admin management permission.
...
- Translation now provides authenticated language discovery, validated source/target translation, same-language identity behavior, a local fallback provider, frontend workspace access, and live certification. `StatChat/scripts/certify_translation.ps1` passes language discovery, glossary translation, same-language handling, invalid-language and empty-input rejection, and authentication requirements; Go tests/vet, the frontend build, and the route audit pass at 155/167.
...
- Wellness workflows now provide tenant-scoped authenticated posts, default seeded content, durable comments, per-user idempotent likes/bookmarks, durable shares, frontend controls, and missing-target/authentication boundaries. `StatChat/scripts/certify_wellness.ps1` passes all seven live PostgreSQL assertions; Go tests/vet, the frontend build, and the route audit advance to 155/167.
- Photo/video/article feed posts now provide authenticated tenant-scoped typed publishing, article title validation, URL-backed media rendering, same-tenant visibility, foreign-tenant isolation, and frontend composer actions. `StatChat/scripts/certify_feed_post_types.ps1` passes all five live PostgreSQL assertions; Go tests/vet, the frontend build, and the route audit remain green at 155/167.
