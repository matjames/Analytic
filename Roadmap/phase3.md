...
- [x] Frontend wired to supported product APIs — repeatable audit verifies 156 typed frontend operations against 168 routes; the 12 uncovered routes are health/readiness, legacy aliases, upload serving, and duplicate task aliases. Secure group/channel member administration is exposed only when the server reports owner/admin management permission.
...
- [x] Collaboration feed and social graph operational — authenticated posts/comments and tenant-scoped interactions are paired with explicit pending/accepted/declined connection requests, recipient-only response controls, reciprocal accepted edges, idempotent requests, reciprocal removal, and self/cross-tenant isolation.
- [x] Wellness interactions operational — wellness authorship is authenticated and tenant-scoped, seeded default content remains available, comments persist with authenticated authorship, likes and bookmarks are per-user idempotent toggles, shares persist durably, and frontend controls are wired to live APIs.
- [x] Photo/video/article feed posts operational — typed article publishing, validated image/video media references, authenticated multipart media upload up to 16 MB, same-tenant feed visibility, frontend composer actions, and media rendering are live-verified.
...
- Translation now provides authenticated language discovery, validated source/target translation, same-language identity behavior, a local fallback provider, frontend workspace access, and live certification. `StatChat/scripts/certify_translation.ps1` passes language discovery, glossary translation, same-language handling, invalid-language and empty-input rejection, and authentication requirements; Go tests/vet, the frontend build, and the route audit pass at 156/168.
- Social-graph policy now provides explicit pending/accepted/declined requests, idempotent duplicate handling, recipient-only accept/decline, reciprocal accepted edges, reciprocal removal, and self/cross-tenant isolation. `StatChat/scripts/certify_social_graph.ps1` passes all seven live PostgreSQL assertions; Go tests/vet, the frontend build, and the route audit pass at 150/162.
- Wellness workflows now provide tenant-scoped authenticated posts, default seeded content, durable comments, per-user idempotent likes/bookmarks, durable shares, frontend controls, and missing-target/authentication boundaries. `StatChat/scripts/certify_wellness.ps1` passes all seven live PostgreSQL assertions; Go tests/vet, the frontend build, and the route audit advance to 156/168.
- Photo/video/article feed posts now provide authenticated tenant-scoped typed publishing, article title validation, authenticated multipart image/video upload up to 16 MB, media rendering, same-tenant visibility, foreign-tenant isolation, and frontend composer actions. `StatChat/scripts/certify_feed_post_types.ps1` passes all six live PostgreSQL assertions; Go tests/vet, the frontend build, and the route audit remain green at 156/168.
- StatCollect cross-module collaboration now uses `statgate-lib/statchat` and `statgate-lib/events`: submissions resolve canonical `obj:statcollect:submission:<id>` conversations, service-authored lifecycle messages use returned conversation IDs, and tenant/workspace/service headers are enforced. The shared event envelope contract and StatChat HTTP boundary are covered by isolated tests; StatCollect `go test ./...` passes.
