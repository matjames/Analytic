...
func TestStatChatIntegrationForwardsLearningIdentity(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if input.ObjectRef != "obj:learning-crm:lead:lead-1" {
	...
	defer server.Close()
	conversation, err := NewStatChatIntegration(server.URL, "internal-key").ensureDiscussion(context.Background(), "Bearer token", "user-1", "tenant-1", "workspace-1", "lead", "lead-1", "Lead")
	if err != nil {
		t.Fatal(err)
	}
